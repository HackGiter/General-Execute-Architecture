...
def apply_rotary_pos_emb(q, k, cos, sin, position_ids, unsqueeze_dim=1, extra=5, training=False):
    cos = cos.unsqueeze(unsqueeze_dim)
    sin = sin.unsqueeze(unsqueeze_dim)
    if training:
        position_ids = position_ids[..., :-extra].expand(extra, -1) + torch.arange(1, extra + 1, dtype=position_ids.dtype, device=position_ids.device).view(-1, 1)
    q_embed = (q * cos[..., -extra:, :]) + (rotate_half(q) * sin[..., -extra:, :]) if not training else (q * cos.squeeze()[position_ids].unsqueeze(unsqueeze_dim)) + (rotate_half(q) * sin.squeeze()[position_ids].unsqueeze(unsqueeze_dim))
    k_embed = (k * cos[..., :-extra, :]) + (rotate_half(k) * sin[..., :-extra, :])
    return q_embed, k_embed

def apply_rotary_pos_emb_vec(v, cos, sin, position_ids, training):
    cos = cos.squeeze(1).squeeze(0)
    sin = sin.squeeze(1).squeeze(0)
    cos = cos[position_ids].unsqueeze(1)
    sin = sin[position_ids].unsqueeze(1)
    v_embed = (v * cos) + (rotate_half(v) * sin) if training else (v * cos[..., -1:, :]) + (rotate_half(v) * sin[..., -1:, :])
    return v_embed
...
class xAdaptiveConfig(LlamaConfig):
    model_type = "xa_model"
    def __init__(
            self,
            vocab_size=32000,
...
class xAdaptiveAttention(nn.Module):
    ...
    def __init__(self, config: xAdaptiveConfig, k:int):
        super().__init__()
        self.config = config
        self.hidden_size = config.hidden_size
        ...
        self.x_embeds = nn.Linear(config.hidden_size, config.hidden_size * (self.k - 1), bias=False)
        # self.x_embeds = nn.Linear(config.hidden_size * 2, config.hidden_size * (self.k - 1), bias=False)
    ...
    def forward(
        ...
        if self.training:
            hidden_states = torch.cat([
                hidden_states[:, 1:],
                self.x_embeds(hidden_states[:, 1:]).view(-1, self.k - 1, self.hidden_size).transpose(1, 0)
            ], dim=0)
        else:
            hidden_states = torch.cat([
                hidden_states[:, -1:],
                # self.x_embeds(hidden_states[:, -2:].view(hidden_states.shape[0], 1, -1)).view(-1, self.k - 1, self.hidden_size)
                self.x_embeds(hidden_states[:, -1:]).view(-1, self.k - 1, self.hidden_size)
            ], dim=1)
        ...
        query_states = query_states.view(q_bsz, -1, self.num_heads, self.head_dim).transpose(1, 2)
        key_states = key_states.view(kv_bsz, -1, self.num_key_value_heads, self.head_dim).transpose(1, 2)
        value_states = value_states.view(kv_bsz, -1, self.num_key_value_heads, self.head_dim).transpose(1, 2)

        if past_key_value is not None:
            kv_seq_len += past_key_value[0].shape[-2]

        cos, sin = self.rotary_emb(value_states, position_ids)
        query_states, key_states = apply_rotary_pos_emb(query_states, key_states, cos, sin, position_ids, extra=self.k, training=self.training)
        ...
        if attn_weights.size() != (q_bsz, self.num_heads, q_len, kv_seq_len) and not self.training:
            raise ValueError(
                f"Attention weights should be of size {(q_bsz, self.num_heads, q_len, kv_seq_len)}, but is"
                f" {attn_weights.size()}"
            )
        ...
        if attn_output.size() != (q_bsz, self.num_heads, q_len, self.head_dim):
            raise ValueError(
                f"`attn_output` should be of size {(q_bsz, self.num_heads, q_len, self.head_dim)}, but is"
                f" {attn_output.size()}"
            )

        attn_output = attn_output.transpose(1, 2).contiguous()
        attn_output = attn_output.reshape(q_bsz, q_len, self.hidden_size)
...
class xAdaptiveDecoderLayer(nn.Module):
    def __init__(self, config: xAdaptiveConfig, id: int, k: int):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.id = id
        self.self_attn = xAdaptiveAttention(config=config, k=k)
...
class xAdaptiveHead(nn.Module):
    def __init__(self, config: xAdaptiveConfig, inp_hid_sz:int=4096, oup_hid_sz:int=4096, intm_hid_sz:int=5120, ratio:float = 0.25) -> None:
        super().__init__()
        self.hidden_size = inp_hid_sz
...
class xAdaptiveModel(xAdaptivePretrainedModel):
    ...
    def __init__(self, config: xAdaptiveConfig, inp_hid_sz: int = 4096, oup_hid_sz: int = 4096, intm_hid_sz: int = 5120, k:int = None):
    ...
    def forward(
        ...
        seq_length = prev_hidden_states.shape[1] + self.k if not self.training else seq_length - 1
            ...
            if position_ids is None:
                ...
                if attention_mask is not None:
                    ...
                    position_ids = torch.arange(
                        past_key_values_length, seq_length + past_key_values_length + (self.k if self.training else 0), dtype=torch.long, device=device
                    )
                position_ids = position_ids.unsqueeze(0).view(-1, seq_length + (self.k if self.training else 0))
            else:
                position_ids = position_ids.view(-1, seq_length).long() if position_ids.shape[-1] == seq_length else position_ids[..., -seq_length:].view(-1, seq_length).long()
